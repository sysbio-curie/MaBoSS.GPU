...
cudaError_t set_internals(const int* indices, int count);
...
__global__ void initialize_initial_state(int trajectories_count, state_t* __restrict__ states, float* __restrict__ times,
										 curandState* __restrict__ rands, float* initial_probas)
{
	auto id = blockIdx.x * blockDim.x + threadIdx.x;
	if (id >= trajectories_count)
		return;
...
{
	initialize_initial_state<<<DIV_UP(trajectories_count, 256), 256>>>(trajectories_count,
																	   states, times, rands, initial_probas);
}
...
__global__ void simulate(float max_time, float time_tick, int internals_count, int trajectories_count,
						 ...
						 trajectory_status* __restrict__ trajectory_statuses)
{
	...
	while (true)
	{
		...
		else
		{
			...
			transition_entropy = compute_transition_entropy(transition_rates, internals_count);
		}
				  ...
				  trajectory_status* trajectory_statuses)
{
	if (discrete_time)
		simulate<true><<<DIV_UP(trajectories_count, 256), 256>>>(
			max_time, time_tick, internals_count, trajectories_count, trajectory_limit, last_states, last_times, rands,
...
void set_boolean_function_variable_values(const float* values) { CUDA_CHECK(set_variables(values)); }

void set_noninternal_indices(const int* indices, int count) { CUDA_CHECK(set_indices(indices, count)); }
