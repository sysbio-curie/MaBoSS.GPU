...
def get_initial_states(cfg, istates):
    ...
    for istate, values in istates.items():
        if 1 in values.keys() and 0 in values.keys():
            ...
            print(
                (
                    "Warning : Initial state probabilities needs to be defined with : "
                    + "[Node].istate = <proba_0> [0], <proba_1> [1];\n"
                    "Inital state definition for node %s will be ignored" % istate
                ),
                file=sys.stderr,
            )
...
def is_trivial_logic_function(node, nodes, variables):
    ...
    if (
        type(up_expr) == TernExpr
        and type(up_expr.cond) == Alias
        and up_expr.cond.name == "logic"
        and type(down_expr) == TernExpr
        and type(down_expr.cond) == Alias
        and down_expr.cond.name == "logic"
        and up_expr.false_branch.evaluate(variables) == 0
        and down_expr.true_branch.evaluate(variables) == 0
    ):
        return True
...
def generate_files(
    bnd_stream, cfg_stream, json_file, runtime_vars_flag, runtime_inter_flag
):
    ...
    generate_tr_cu_file(
        tr_cu_file, nodes, variables, cfg_program, runtime_vars_flag, runtime_inter_flag
    )
...
if __name__ == "__main__":
    ...
    generate_files(
        ...
        cfg_stream,
        args.json_file,
        args.runtime_variables,
        args.runtime_internals,
    )
